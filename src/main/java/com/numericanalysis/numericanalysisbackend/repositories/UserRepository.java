...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Transactional
    @Modifying
    @Query("UPDATE users u SET u.email = ?2, u.password = ?3, u.age = ?4, u.nickname = ?5, u.activity = ?6, u.description = ?7 WHERE u.email = ?1")
    ...
    int edit(@Param("email")String email, @Param("newEmail")String newEmail, @Param("newPassword")String newPassword,
              @Param("age") int age, @Param("nickname")String nickname, @Param("activity")String activity,
